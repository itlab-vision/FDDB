...
my $imDir = "/home/kate/OriginPic/"; 
# where the folds are
my $fddbDir = "/home/kate/FDDB-folds/"; 
# where the detections are
my $detDir = "/home/kate/CascadeResult/"; 
###########################
...
my $format = ".jpg";  #формат в зависимости от формата картинок 
...
foreach my $fi (1..10){
  my $foldFile = sprintf("%sfold-%02d-out.txt", $detDir, $fi);
  system("cat $foldFile >> $detFile");
  $foldFile = sprintf("%sFDDB-fold-%02d.txt", $fddbDir, $fi);
  system("cat $foldFile >> $listFile");
  $foldFile = sprintf("%sFDDB-fold-%02d-ellipseList.txt", $fddbDir, $fi);
...
system($evaluateBin, "-a", $annotFile, "-d", $detFile, "-f", $detFormat, "-i", $imDir, "-l", $listFile, "-r", $detDir, "-z", $format);
